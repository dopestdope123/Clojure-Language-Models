...
(defn score-next-state-word [curr-state next-state next-word t-dists o-dists]
  (+ 
...
(defn compute-next-observation-marginal [curr-state next-observation t-dists o-dists]
  (logsumexp (map (fn [x] (score-next-state-word curr-state x next-observation 
    t-dists o-dists))
       (rest hidden-states))))

;; Problem 3  - score-next-states-words

(defn score-next-states-words [curr-state next-states next-words t-dists o-dists]
  (if (empty? next-words)
    0
    (+ 
...
(defn compute-next-words-marginal [curr-state next-words t-dists o-dists]
  (if (empty? next-words)
    0
    (logsumexp 
    (list 
...
(defn compute-hidden-prior [list-of-states t-dists]
...
(defn compute-likelihood-of-words [list-of-states list-of-words o-dists]
  (if (empty? list-of-states)
    0
...
(defn compute-hidden-posterior [list-of-states list-of-words t-dists o-dists]
...
(def compute-next-words-marginal-mem (memoize (fn 
  ...
  (if (empty? next-words)
    0
    (logsumexp 
    (list (+ (score-next-state-word curr-state (first (rest hidden-states)) 
      (first next-words) t-dists o-dists) 
      (compute-next-words-marginal (first (rest hidden-states)) (rest next-words) 
        t-dists o-dists))
    (+ (score-next-state-word curr-state (first (rest (rest hidden-states))) 
