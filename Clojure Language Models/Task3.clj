...
(defn get-count-of-word [w word-tokens count]
    (if (empty? word-tokens)
      ...
      (if (= w (first word-tokens))
        (inc (get-count-of-word w (rest word-tokens)))
...
(def moby-words-frequencies(get-word-counts moby-vocab moby-word-tokens))
...
(defn sample-uniform-BOW-sentence [n vocabulary]
  (if (= n 0)
    '()
    (cons (sample-categorical vocabulary (create-uniform-distribution vocabulary))
          (sample-uniform-BOW-sentence (- n 1) vocabulary))))
...
(def moby-word-probabilities (normalize moby-words-frequencies))

;; Problem 9: Define a function lookup-probability
(defn lookup-probability [w outcomes probs]
  (if (not (member-of-list? w outcomes))
    0
    (if (= w (first outcomes))
      (first probs)
